...
def experiment(final_params):
    ...
    for num_run in range(runs):
        ...
        if not hasattr(final_params, 'test_set_path'):
            # default data path if none specified 
            final_params.test_set_path = '/data/imagenet_data/Data/CLS-LOC'
        ...
        if hasattr(final_params, 'debug_sample_per_cls'):
            cls_limit = final_params.debug_sample_per_cls
        else: cls_limit = None
        ...
        final_params.samples_per_cls = 1300
        ...
        # Check if specified task partition is valid 
        assert((sum([a*b for a,b in num_task_cls_per_task]))<=len(class_order))
        print('Constructing the tasks: ')
        task_st, class_st = 0,0
        ...
        data, labels = load_data(fpath)
        
        ### Training ### 
        # Create the CL framework
        continual = Continual(**vars(final_params))
        task_st, class_st = 0,0
        print('Loading task [',end=' ')
        for (num_task, cls_per_task) in num_task_cls_per_task:
            for task_id in range (task_st,task_st+num_task):
                for label in range (class_st, class_st + cls_per_task):
                    cnt = 0
                    print(f'({label}th:{class_order[label]})',flush=True,end=',')
                    for data_path in data[class_order[label]]:
                        ...
                        continual.send_stream_data(img, class_order[label], task_id) # class_order[label]
                        if cls_limit: 
                            cnt+=1 
                            if cnt >= cls_limit: break
                        del img,file
                class_st+=cls_per_task  
