...
class ReplayCIFAR(Dataset):
    ...
    def shm_init(self): 
        ...
        while True: 
            try:
                self.data_shm_name = shared_memory.ShareableList([f'cifar100_{str(self.device)[-1]}_replay_0000000000000000_{self.postfix}']*self.rb_size)#,name=f'replay_shm_names_{self.posifix}') 
        ...
        for i in range(self.rb_size):
            while True:
                try:
                    item = shared_memory.SharedMemory(create=True,name=f'cifar100_{str(self.device)[-1]}_replay_{i}_{self.postfix}', size=self.vec_size*self.coefficient)
                    break
                except (RuntimeError, FileExistsError): 
                    self.postfix+=1
                    continue 
        ...
        self.targets = shared_memory.ShareableList([-1]*self.rb_size,name=f'cifar100_{str(self.device)[-1]}_replay_targets_{self.postfix}')
        self.targets_shm_name = self.targets.shm.name
        self.filenames = shared_memory.ShareableList(['0000_0000.png']*self.rb_size,name=f'cifar100_{str(self.device)[-1]}_replay_filenames_{self.postfix}')
    ...
    def shm_resize(self,new_size,delete=False): 
        ...
        if new_size > len(self.data_shm):
            ...
            new_name = shared_memory.ShareableList([f'cifar100_{str(self.device)[-1]}_replay_000000_{self.postfix}']*new_size,name=f'cifar100_{str(self.device)[-1]}_replay_shm_names_{self.postfix}')
            new_files = shared_memory.ShareableList(['00000_00000.png']*new_size,name=f'cifar100_{str(self.device)[-1]}_replay_filenames_{self.postfix}')
            new_targets = shared_memory.ShareableList([-1]*new_size,name=f'cifar100_{str(self.device)[-1]}_replay_targets_{self.postfix}')
            ...
            for i in range(len(self.data_shm),new_size): 
                create = True
                while True:
                    name = f'cifar100_{str(self.device)[-1]}_replay_{i}_{self.postfix}'
                    try: 
    ...
    def __setitem__(self,idx,value):
        ...
        if type(idx) == int: 
            ...
            if vec.size > data_shm.size: 
                self.clean(idx)
                self.data_shm[idx] = shared_memory.SharedMemory(create=True, name=f'cifar100_{str(self.device)[-1]}_replay_{idx}_{self.postfix}',size=vec.size)
