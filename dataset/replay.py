...
EMPTY_LIST = 56 
ELEMENT_SIZE = 8 
CIFAR_IMAGE_SIZE = 3105*4
TINY_IMAGE_SIZE = 3105*5
IMAGENET_IMAGE_SIZE = 224*224*3*4 + 32

ULBANSOUND8K_VEC_SIZE = 30000*4
DAILYNSPORTS_VEC_SIZE = 5625*4
AUDIOSET_VEC_SIZE = 64064*4

CIFAR_IMG_SHAPE = (32,32,3)
TINY_IMG_SHAPE = (64,64,3)
IMG1000_IMG_SHAPE= (224,224,3)
ULBANSOUND8K_VEC_SHAPE = (100,100,3) # (64,64,3)
TWENTYNEWS_VEC_SHAPE = (100,300)
AUDIOSET_VEC_SHAPE = (1001,64)
DAILYNSPORTS_VEC_SHAPE = (125,45)
AUDIOSET_VEC_SHAPE_2 = (256,250)
IMG_DTYPE = 'uint8'
VEC_DTYPE = 'float32'
...
class ReplayDataset(Dataset):
    ...
    def __init__(self, rb_path, rb_size, transform, sampling, agent,dataset='cifar100',device='cpu',postfix=None):
        ...
        # if self.test_set in ['imagenet1000']:
        ...
        if dataset == 'cifar100':
            self.vec_shape = CIFAR_IMG_SHAPE
            self.vec_dtype = IMG_DTYPE
            self.vec_size = CIFAR_IMAGE_SIZE
        elif dataset == 'tiny_imagenet':
            self.vec_shape = TINY_IMG_SHAPE
            self.vec_dtype = IMG_DTYPE
            self.vec_size = TINY_IMAGE_SIZE
        elif dataset == 'dailynsports':
            self.vec_shape = DAILYNSPORTS_VEC_SHAPE
            self.vec_dtype = VEC_DTYPE
            self.vec_size = DAILYNSPORTS_VEC_SIZE
        elif dataset == 'urbansound8k':
            self.vec_shape = ULBANSOUND8K_VEC_SHAPE
            self.vec_dtype =  IMG_DTYPE
            self.vec_size = ULBANSOUND8K_VEC_SIZE
        elif dataset == 'shakespeare':
            self.vec_shape = SHAKESPEARE_VEC_SHAPE
            self.vec_dtype = VEC_DTYPE
            self.vec_size = SHAKESPEARE_VEC_SIZE
        elif dataset == 'twentynews':
            self.vec_shape = TWENTYNEWS_VEC_SHAPE
            self.vec_dtype = VEC_DTYPE
            self.vec_size = TWENTYNEWS_VEC_SIZE
        elif dataset == 'imagenet1000':
            self.vec_shape = IMG1000_IMG_SHAPE
            self.vec_dtype = IMG_DTYPE
            self.vec_size = IMAGENET_IMAGE_SIZE
            self.set_transform = transforms.Compose([
                                            transforms.RandomResizedCrop(224),
                                            transforms.RandomHorizontalFlip(),
                                            transforms.ColorJitter(brightness=63/255),
                                            ])
            self.get_transform =transforms.Compose([
                                            transforms.ToTensor(),
                                            transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                            std=[0.229, 0.224, 0.225])
                                            ])
        elif dataset == 'audioset':
            self.vec_shape = AUDIOSET_VEC_SHAPE
            self.vec_dtype = VEC_DTYPE
            self.vec_size = AUDIOSET_VEC_SIZE
        else:
            self.vec_shape = CIFAR_IMG_SHAPE
            self.vec_dtype = IMG_DTYPE
            self.vec_size = CIFAR_IMAGE_SIZE
    ...
    def generate_postfix(self):
        ...
        return self.postfix
    def shm_init(self): 
        print(f'shm_init, postfix: {self.postfix}')
        ...
        # self.data_shm_idx = [k for k in range(self.rb_size)]
    ...
    def shm_resize(self,new_size,delete=False): 
        ...
        #df.write(f'shm_resize: {self.rb_size} -> {new_size}, delete = {delete}\n')
        #df.close()       
        if new_size == self.rb_size:
            print('NO RESIZE')
            return 3
        exit_code = -1
        print(f'shm_resize: {self.rb_size} -> {new_size}, delete = {delete}')
        # df = open(DEBUG_FILE,'a')
        # df.write(f'shm_resize: {self.rb_size} -> {new_size}, delete = {delete}\n')
        # df.close()
        ...
        if new_size > len(self.data_shm):
            ...
            # new_files[:len(self.filenames)] = filename_copy
            ...
            # multi_task_sample_update_to_RB(self,None,*[False])
            # self.filled[0] = new_size
            # df.write(str(self.len_per_cls)+'\n')
            # df.write(str(self.offset)+'\n')
            # df.write(str(self.filenames)+'\n')   
            # df.close()       
            return 2
    ...
    #### BUG #### 
    # when class N contains less than the budget allocated, offsets should be adjusted
        #df = open(DEBUG_FILE,'a')
        #df.write(f'resize sampling, mem_per_cls={mem_per_cls}\n')
    ...
    def __setitem__(self,idx,value):
        ...
        if type(idx) == int: 
            ...
            if self.test_set == 'imagenet1000': 
                assert(self.set_transform)
                vec = self.set_transform(vec)
                vec = np.asarray(vec)
    ...
    def getitem_offline(self, idx):
        ...
        if type(idx) == int:  
            ...
            # return old_idx, data, self.targets[old_idx], self.filenames[old_idx]
    ...
    def get_meta(self): 
        ...
        if self.test_set == 'imagenet1000':
            meta['transform'] = self.set_transform
        else:
    ...
    def getitem_online(self, idx):
        ...
        if self.agent in ["der","derpp_distill","derpp"]:
            ...
            return idx, vec, label, logit, data_id
        else:
